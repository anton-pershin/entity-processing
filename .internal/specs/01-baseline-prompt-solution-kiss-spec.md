...
7. **Validation of model output.** The implementation must normalize model output before writing it: retain the first entity for each unique `entity_id`, discard entities with unknown types or sentiments, and discard relations with unknown types or endpoints that do not resolve to retained entities. A response is considered unparseable only when no valid JSON object can be extracted after removing an optional Markdown code fence and surrounding explanatory text; such a response produces empty lists for that document.
...
5. **Response-format parsing test.** Return raw JSON, fenced JSON, fenced JSON followed by prose, and prose surrounding JSON. Assert that each valid JSON object is extracted and normalized. Return genuinely malformed output and assert that the document receives empty lists and the failure is logged.
6. **Output validation test.** Return responses containing an unknown entity type, unknown sentiment, unknown relation type, duplicate entity IDs, and relation endpoints that do not exist. Assert that invalid entities and relations are discarded, only the first valid occurrence of each entity ID is retained, and valid remaining data is retained.
7. **Per-document failure-isolation test.** Make the mock client fail or return malformed output for one document and succeed for a later document. Assert that the failed document receives empty lists, an error is logged, and the later document is still processed and written.
8. **Configuration test.** Run the script with non-default label lists and a non-default model value supplied through Hydra overrides. Assert that the prompt and output use those overrides rather than hardcoded defaults.
9. **Validation-contract test.** Run the produced JSONL through the same structural assumptions as the validation service and assert that every output `doc_id` corresponds to an input document, every entity ID is document-local and unique, and every relation endpoint resolves to an entity in that document.
10. **Regression test for the baseline invocation.** Execute the script with the exact argument shape used by the validation service (`input`, `output`, `entity_types`, `relation_types`, and `sentiment_types`) and assert successful completion and parseable JSONL output.
...
4. **Output validation:** define typed internal models or equivalent validation helpers for the LLM result. First extract the first valid JSON object from raw content, optionally removing a Markdown code fence and surrounding explanatory text; then retain the first entity for each unique `entity_id`, discard entities with unknown types or sentiments, and discard relations with unknown types or unresolved endpoints. If no valid JSON object can be extracted, produce empty lists for that document.
